...
from framework.auth.principals import Principal, create_user_principal
...
async def get_principal(
    ...
    try:
        ...
        if token_use == "access":
            ...
            from framework.auth.principals import create_service_principal
...
def require_role(required_role: str):
    ...
    def role_checker(principal: Principal = Depends(get_principal)) -> Principal:
        if not principal.has_role(required_role):
            ...
            raise HTTPException(status_code=403, detail=f"Insufficient role. Required: {required_role}")
        return principal

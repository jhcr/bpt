...
from ..middleware.auth_jwt import get_principal, require_user_read, require_usersettings_read, require_usersettings_write
...
from ..schema.user_schemas import UserResponse, UserSettingsResponse, UpdateUserSettingsRequest
...
async def get_current_user(
    ...
    try:
        user_data = await get_user_uc.execute(principal=principal)
...
async def update_user_settings(
    ...
    try:
        ...
        if "version conflict" in str(e).lower():
            raise HTTPException(status_code=409, detail="Version conflict - settings were modified by another process")

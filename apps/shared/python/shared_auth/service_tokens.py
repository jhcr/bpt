...
class ServiceTokenClient:
    ...
    def __init__(self, auth_base: str, client_id: str, client_secret: str, 
                 sub_spn: str, scope: str):
    ...
    def get(self, actor_sub: Optional[str] = None, 
            actor_scope: Optional[str] = None, 
            actor_roles: Optional[List[str]] = None) -> str:
        """
    ...
    def _fetch_token(self, actor_sub: Optional[str], 
                    actor_scope: Optional[str], 
                    actor_roles: Optional[List[str]]) -> ServiceTokenResponse:
        ...
        try:
            response = httpx.post(
                f"{self.auth_base}/auth/svc/token",
                json=payload,
                timeout=5.0
            )
            ...
            raise ServiceTokenError(f"Failed to fetch service token: {e}")
        except httpx.HTTPStatusError as e:
            raise ServiceTokenError(f"Service token request failed: {e.response.status_code} {e.response.text}")

    def _make_cache_key(self, actor_sub: Optional[str], 
                       actor_scope: Optional[str], 
                       actor_roles: Optional[List[str]]) -> str:
...
class ServiceTokenHttpClient:
    ...
    def __init__(self, service_token_client: ServiceTokenClient, 
                 base_url: str, timeout: float = 5.0):
        self.svc_token = service_token_client
        self.base_url = base_url.rstrip("/")
        self.timeout = timeout

    async def get(self, path: str, actor_sub: Optional[str] = None, 
                  actor_scope: Optional[str] = None, 
    ...
    async def post(self, path: str, actor_sub: Optional[str] = None, 
                   actor_scope: Optional[str] = None, 
                   actor_roles: Optional[List[str]] = None,
                   **kwargs) -> httpx.Response:
    ...
    async def put(self, path: str, actor_sub: Optional[str] = None, 
                  actor_scope: Optional[str] = None, 
                  actor_roles: Optional[List[str]] = None,
                  **kwargs) -> httpx.Response:
    ...
    async def delete(self, path: str, actor_sub: Optional[str] = None, 
                     actor_scope: Optional[str] = None, 
                     actor_roles: Optional[List[str]] = None,
                     **kwargs) -> httpx.Response:

...
from application.ports.session_repository import CipherSessionRepository
from domain.entities.session import CipherSession
...
class CreateCipherSessionUseCase:
    ...
    async def execute(self) -> CipherSession:
        """
        Create a new cipher session

        Returns:
            CipherSession domain entity
        """
        try:
            # Generate session ID
            sid = str(uuid.uuid4())
            ...
            raise CipherSessionError(f"Failed to create cipher session: {e}") from e
